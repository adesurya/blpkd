...
def _safe_device(requested: str) -> str:
    """
    Tentukan device yang aman tanpa import torch.
    Dipanggil saat module load — harus tidak boleh crash.
    - Jika diminta 'cpu' → return 'cpu'
    - Jika diminta 'cuda' → coba cek torch, fallback 'cpu' jika tidak ada
    """
...
class AppSettings(BaseSettings):
    ...
    # ── Compression ───────────────────────────────────────
    # Default: libx264 (CPU) — override ke h264_nvenc di .env jika GPU
    ...
    # ── Computed device (resolved saat pertama kali diakses) ──
    @property
    def yolo_device_resolved(self) -> str:
        return _safe_device(self.YOLO_DEVICE)

    @property
    def insightface_device_resolved(self) -> str:
        return _safe_device(self.INSIGHTFACE_DEVICE)
